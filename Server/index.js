...
function calc_bus_risk(number_masks, people_risk_scores, duration) {
  // console.log(number_masks, people_risk_scores, duration)
  let total_risk = 1;
  ...
  const mask_risk = Math.max(prop_mask * (17.4 / 3.1), 1);
  total_risk /= mask_risk;

  // console.log(total_risk)

  const duration_prop = Math.min(duration / 50, 1);
...
function calc_person_risk(last_ride_risk, historical_risk) {
  const risks = historical_risk.concat([last_ride_risk]);
  console.log(risks);
  total_risk = 1;
...
express()
  ...
  .get("/getColor", async function (req, res) {
    ...
    res.setHeader(
      ...
      "https://safetravels.macrotechsolutions.us"
    );
    res.setHeader(
  ...
  .get("/hardwareConnect", async function (req, res) {
    ...
    myVal = await database
      .ref(`vehicles/${type}/${vehicleid}/times`)
      .once("value");
    myVal = myVal.val();
    ...
    for (key in myVal) {
      if (
        Date.parse(datetime) > Date.parse(myVal[key].starttimes) &&
        Date.parse(datetime) < Date.parse(myVal[key].endtimes)
      ) {
        //timeid = key;
      }
    }
    ...
    if (exist == false) {
      ...
      await database
        .ref(`vehicles/${type}/${vehicleid}/times/${timeid}/seating`)
        .push(seatingPut);
    }
    res.send("success");
  })
    ...
    for (key in myVal) {
      for (key2 in myVal[key].times) {
        ...
        if (starttimes != null && starttimes != "" && starttimes != "null") {
          if (
            Date.parse(myVal[key].times[key2].starttimes) <
            Date.parse(starttimes)
          ) {
            add = false;
          }
        }
        if (endtimes != null && endtimes != "" && endtimes != "null") {
          if (
            Date.parse(myVal[key].times[key2].endtimes) > Date.parse(endtimes)
          ) {
            add = false;
          }
        }
  ...
  .post("/getSeating", async function (req, res) {
    ...
    let dimensions = await database
      .ref(`vehicles/${type}/${vehiclekey}`)
      .once("value");
    dimensions = dimensions.val();
    dimensions = dimensions.dimensions;

    myVal = await database
      .ref(`vehicles/${type}/${vehiclekey}/times/${timekey}/seating`)
      .once("value");
    myVal = myVal.val();
    for (key in myVal) {
    ...
    let editPosition =
      parseInt(bestPosition.split(",")[1]) *
        parseInt(dimensions.split(",")[0]) +
      parseInt(bestPosition.split(",")[0]);
  ...
  .post("/maskDetection", async function (req, res) {
    ...
    await database
      .ref(`vehicles/${type}/${vehicleid}/times/${timeid}/masks`)
      .set(masks);
    await database
      .ref(`vehicles/${type}/${vehicleid}/times/${timeid}/nomasks`)
      .set(nomasks);
    res.send("success");
  })
  .post("/coughIncrement", async function (req, res) {
    ...
    const val = await database
      .ref(`vehicles/${type}/${vehicleid}/times/${timeid}`)
      .once("value");
    if (typeof val.val().coughs !== "number") {
      await database
        .ref(`vehicles/${type}/${vehicleid}/times/${timeid}/coughs`)
        .set(1);
    } else {
      await database
        .ref(`vehicles/${type}/${vehicleid}/times/${timeid}/coughs`)
        .set(1 + val.val().coughs);
    }
    res.send("success");
  })
  .post("/rideFinished", async function (req, res) {
    ...
    let duration = 0;
    let myVal = await database
      .ref(`vehicles/${type}/${vehicleid}/times/${datetimeid}`)
      .once("value");
    myVal = myVal.val();
    ...
    for (key in seating) {
        ...
        .then(function (snapshot) {
          if (snapshot.exists()) {
            snapshot.forEach(async function (childSnapshot) {
              let vehicleid_s = childSnapshot.val().vehicleid;
              let type = childSnapshot.val().type;
              let timeid = childSnapshot.val().timeid;
              if (vehicleid_s == vehicleid && timeid == datetimeid) {
                return
              }
              let myVal4 = await database
                .ref(`vehicles/${type}/${vehicleid_s}/times/${timeid}`)
                .once("value");
              myVal4 = myVal4.val();
              console.log(myVal4);
              let risk = myVal4.risk;
              historical_risk.push({ [user]: myVal4.risk });
            });
          } else {
      ...
      let starttime = Date.parse(myVal.starttimes);
      let endtime = Date.parse(myVal.endtimes);
      duration = (endtime - starttime) / (1000 * 60);
    }

    console.log("MASKS ---------", masks);
    console.log("people_risk_scores ---------", people_risk_scores);
    console.log("duration ---------", duration);

    const bus_risk = calc_bus_risk(masks, people_risk_scores, duration);
    await database
      .ref(`vehicles/${type}/${vehicleid}/times/${datetimeid}/risk`)
      .set(bus_risk);
    console.log("BUS RISK ---------", bus_risk);

    console.log(historical_risk);
    const hist_by_user = {};
    for (let event of historical_risk) {
      const user = Object.keys(event)[0];
      if (user in hist_by_user) {
        hist_by_user[user].push(historical_risk[user]);
      } else {
        hist_by_user[user] = [event[user]];
        console.log(hist_by_user);
      }
    }

    for (let user in hist_by_user) {
      const new_risk = calc_person_risk(bus_risk, hist_by_user[user]);
      await database.ref(`users/${user}/risk`).set(new_risk);
    }
